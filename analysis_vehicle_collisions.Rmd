...
#Redo Plot 3 - Easier to show this using a facetted bar plot
...
g1 <-ggplot(aes(x=C_CONF, fill = factor(M,labels=c("Non-Deadly",
    ...
    scale_fill_manual(values=c("black", "red"), #change the colors - red is Death :(
                      name = "Collision Severity (M)") + #change legend title
    ggtitle("Total Accidents by Collision Types for Cars") + 
    xlab("Collision Type\n(C_CONF)") + 
    ylab("Total Accidents Count") +
    coord_flip() +
    theme(legend.position = "top",axis.text.x = element_text(size = 8, 
      angle = 0, vjust = 0.5, hjust=1),plot.title = element_text(lineheight=.8, 
      face="bold"))
...
g2 <- ggplot(aes(x=C_CONF, fill = factor(M,labels=c("Non-Deadly",
    ...
    scale_fill_manual(values=c("black", "red"), #change the colors - red is Death :(
                      name = "Collision Severity (M)") + #change legend title
    ggtitle("Total Accidents by Collision Type for Non-Car Vehicle Types") + 
    xlab("Collision Type\n(C_CONF)") + 
    ylab("Total Accidents Count") +
    coord_flip() +
    theme(legend.position = "top",axis.text.x = element_text(size = 8, 
...
#### Head-on collisions (31) are the deadliest type of collisions for many vehicle types which is not a surprise, but its interesting to see that tractors (08) in particular have a large proportion of deadly head-on collisions. You can also see that vast majority of accidents involve cars (or other light duty vehicles).
