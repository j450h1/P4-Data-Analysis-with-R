...
library(ggplot2)
library(scales) #needed for percent y-scale of 2nd plot
library(plyr)
...
### Plot Two
```{r, echo=FALSE, Plot_Two, out.width='1200px', dpi=200}
##OLD PLOT 2 - scroll below for new one
...
# ----------------------------------------------------------------------
### NEW Plot 2
#Create new vehicle type numeric variable so its easy to subset and show only a few vehicle types
# learned that I must convert to character first or else (11 and 10) get converted to 1
df$V_TYPE_NUM <- as.numeric(as.character(df$V_TYPE))
#table(df$V_TYPE,df$P_SAFE) - Too many levels
remove_safe <- df$P_SAFE %in% c("NN","QQ","UU") 
mdf <- subset(df,C_CONF != "UU" & C_CONF!= "QQ" & df$V_TYPE_NUM <= 8 & 
              !remove_safe) #Not showing vehicle types with very low counts
mdf <- droplevels(mdf)
levels(mdf$V_TYPE) <- c("01 - CARS","05 - VAN","06 - SMALL TRUCKS", "O7 - LARGE TRUCKS"      ,"08 - TRACTORS")
#Relevel into Seat Belt vs No-Seat Belt
mdf$P_SAFE <- ifelse(mdf$P_SAFE == "01", "NO", "YES")
mdf$P_SAFE <- factor(mdf$P_SAFE)
###Non Deadly Collisions
g1 <- ggplot(aes(x=V_TYPE, fill=P_SAFE), data = subset(mdf, M == 0)) + 
  geom_bar(aes(y = (..count..)/sum(..count..)), position = "fill") + #height is 100 %
  scale_y_continuous(breaks = seq(0,1,0.05), labels = percent) +
  scale_fill_manual(values=c("red", "green"), #change the colors - red is bad
                    name = "Seat Belt Used?") + 
  ggtitle("Seat Belt Usage by Vehicle Type\n(Non-Deadly Collisions)") + 
  xlab("Vehicle Type\n(V_TYPE)") + 
  ylab("Percent of Non-Deadly Collisions") +
  theme(legend.position = "top", axis.text=element_text(size=8))
###Deadly Collisions  
g2 <- ggplot(aes(x=V_TYPE, fill=P_SAFE), data = subset(mdf, M == 1)) + 
  geom_bar(aes(y = (..count..)/sum(..count..)), position = "fill") + #height is 100 %
  scale_y_continuous(breaks = seq(0,1,0.05), labels = percent) + 
  scale_fill_manual(values=c("red", "green"), #change the colors - red is bad
                    name = "Seat Belt Used?") + 
  ggtitle("Seat Belt Usage by Vehicle Type\n(Deadly Collisions)") + 
  xlab("Vehicle Type\n(V_TYPE)") + 
  ylab("Percent of Deadly Collisions") +
  theme(legend.position = "top", axis.text=element_text(size=8))
#multiplot(g1,g2, cols = 2)  doesn't look good in html
```

```{r}
#Would have liked to plot side by side, but just too squeezed in
g1
```
...
#### We can see that deadly collisions involved a larger proportion of drivers who didn't wear a seatbelt (or other safety device) compared to non-deadly collisions and this is consistent across vehicle types. For non-deadly collisions, the largest proportion of drivers who didn't wear their seatbelts were drivers of large trucks (07) and tractors (08). However, for deadly collisions, drivers of vans (05) and small trucks (06) had the largest proportion of deaths. 

### Plot Three
```{r, echo=FALSE, Plot_Three, out.width='1200px', dpi=200}
###OLD PLOT THREE - see below for new one###
...
# ---------------------------------------------------------------------------------
#Redo Plot 3 - Easier to show this using a facetted bar plot/histogram
#Plot Car separately - tried plotting all on log scale together, but easier to see contrast if its separate
### Car Plot
g1 <-ggplot(aes(x=C_CONF, fill = factor(M,labels=c("Non-Deadly",
    "Deadly"))), data=subset(df,C_CONF != "UU" & C_CONF!= "QQ")) + #only look at a few vehicle types
    geom_histogram() +
    scale_fill_manual(values=c("black", "red"), #change the colors - red is Death :(
                      name = "") + #no legend title name needed
    ggtitle("Total Collisions by Collision Type\n(Cars)") + 
    xlab("Collision Type\n(C_CONF)") + 
    ylab("Number of Collisions") +
    coord_flip() +
    theme(legend.position = "top", axis.text=element_text(size=8))

bdf <- subset(df,C_CONF != "UU" & C_CONF!= "QQ" & df$V_TYPE_NUM <= 8 & df$V_TYPE_NUM != 1)
### OTHER VEHICLES PLOT
#Drop the old levels which have been subsetted out above
bdf <- droplevels(bdf)
#Re-level vehicle types
levels(bdf$V_TYPE) <- c("05 - VAN","06 - SMALL TRUCKS", "O7 - LARGE TRUCKS",
                        "08 - TRACTORS")
# Get the levels for type in the required order
g2 <- ggplot(aes(x=C_CONF, fill = factor(M,labels=c("Non-Deadly",
    "Deadly"))), data=bdf) + #only look at a few vehicle types
    geom_histogram() +
    scale_fill_manual(values=c("black", "red"), #change the colors - red is Death :(
                      name = "") + #no legend title name needed
    ggtitle("Total Collisions by Collision Type\n(Other Vehicle Types)") + 
...
#### Rear-end collisions (21) are by far the most common type of collisions across many vehicle types. However, head-on collisions (31) are the deadliest type of collisions. Its interesting to see that tractors in particular have a large proportion of deadly head-on collisions. You can also see that vast majority of accidents involve cars (i.e. light duty vehicles) and they are literally on a different scale compared with other vehicle types.
