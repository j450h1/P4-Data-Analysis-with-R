...
datatable(head(df[,2:13]), class = 'cell-border stripe', rownames = FALSE, 
          caption = 'Table 1: Collision Level Variables (head)', options = list(dom = 't'))
```
...
datatable(head(df[,14:17]), class = 'cell-border stripe', rownames = FALSE,
          caption = 'Table 2: Vehicle Level Variables (head)', options = list(dom = 't'))
```
...
datatable(head(df[,17:23]), class = 'cell-border stripe', rownames = FALSE,
          caption = 'Table 3: People Level Variables (head)', options = list(dom = 't'))
```
...
Looks like distribution across the weekdays is bimodal throughout the "sleeping" hours (Midnight to 7 AM), while the distribution on the weekend is almost identical in shape for Saturday & Sunday (being nearly unimodal for this same hour range).
...
Definitely, there doesn't seem to be any linear relationsips here. Except for a decent correlation between Weather & Road Surface (*C_WTHR_NUM* & *C_RSUR_NUM*) of `r round(cor(df$C_WTHR_NUM,df$C_RSUR_NUM),2)` 

```{r}
qplot(C_RSUR_NUM,C_WTHR_NUM, data = df, geom="jitter", alpha = 0.5)
```
...
Let's see what other relatonships we can explore further.
...
Looks like many deadly crashes had no safety device such as a seat belt used (pink) (this is consistent across the 5 major types of vehicles).
...
Nothing out of the ordinary, we can see that school bus drivers (6) and RV drivers (9) skew towards older drivers.
...
g1 <- ggplot(aes(x=P_AGE), data = subset(df,P_AGE_NUM >= age_initial & 
  ...
  annotate("text", label = paste("Age 19: ",as.character(most_deaths)), 
    x= age_most - age_initial, hjust = 0, y = most_deaths + 100,color= "black") + #Show the exact number
  annotate("text", label = paste("Age 46: ",as.character(older_deaths)), 
    x= age_old - age_initial, hjust = 0, y = older_deaths + 100, color = "black") + #Show the exact number
...
g2 <- ggplot(aes(x=P_AGE), data = subset(df,P_AGE_NUM >= age_initial & 
  ...
  xlab("Driver's Age (P_AGE)") + 
  scale_y_discrete(breaks = seq(0,95,5), labels = seq(0,95,5)) +
  scale_x_discrete(breaks = seq(15,90,5), labels = seq(15,90,5)) + 
...
#multiplot(g1,g2,2) I want to have both plots at their full size and not use this
```
...
g1 <- ggMMplot(mdf$V_TYPE[mdf$M==1],mdf$P_SAFE[mdf$M==1], "Vehicle Type (V_TYPE)", 
        "Safety Device Used (P_SAFE)","Safety Device Used by Vehicle Type
        \n(Deadly Collisions)")
g2 <- ggMMplot(mdf$V_TYPE[mdf$M==0],mdf$P_SAFE[mdf$M==0], "Vehicle Type (V_TYPE)", 
        "Safety Device Used (P_SAFE)", "Safety Device Used by Vehicle Type
        \n(Non-Deadly Collisions)")
...
g2 + theme(legend.position = "none",axis.text.x = element_text(size = 8, 
    ...
    annotate("text", label = "Safety Device Used", x=0.1 , hjust=0, y=0.5, 
             color= "black", size=10) +
    annotate("text", label = "Light Duty Vehicle(e.g. Cars) (1)", x=0.5 , 
             hjust=0, y=1.05, color= "black", size=8) +
...
### Head-on collisions (31) are the deadliest type of collisions for many vehicle types which is not a surprise, but its interesting to see that tractors in particular have a large proportion of head-on collisions. You can also see that vast majority of accidents involve cars (or other light duty vehicles).
...
Learned that I would need to be very careful when i subset data from a previous year, I didn’t need to remove unknown variables for C_SEV because there was none for 2011, but that’s not necessarily true for other years. It is best to assume that the data has this missing data even if it doesn’t to make the code reproducible.

### Initial Bias

Its also tempting to stop exploring the data when you arrive at the answer that you expected. Its important to keep going as things may not be as they seem from just the initial analysis. 

### Speed of Execution

It would have been better to work with a subset of the data. Actually for the pairwise plot, I sed a sample of 10,000 records and most of the plots showed the same pattern. It took several minutes to run this code even with 10,000 records. At least for the initial exploration, a smaller sample would have sufficed.

### Summary vs Plots

I learned that a lot of the information about the dataset could be gleaned just from the summary output. However, many of the visualizations helped confirm these initial findings but more importantly it helped in finding outliers and unusual relationships that were missed if you looked just at the summary. For example, it would not be able to see that oder teenagers and elderly drivers are disproportionally responsible for deadly collisions.

### Choosing the right plot

Very important to choose the right type of plot or else you can get misleading results. For example, comparing weather and road surface by a simple scatterplot suffers from overplaying and shows that wet roads and windy weather is associated with deadly crashes. But when you use jitter, the pattern is much more clear and you can see that this is not necessarily the trust.

### Predictive Model

Finally, once I complete the machine learning course, i can explore what model if any would help predict deadly crashes based on the provided features as the linear model doesn’t seem appropriate.

###Plotting in R vs HTML Output
